...
from app.models import db, BusinessTask, TaskInstance, InsightLog

TASKS_MD_PATH = os.path.join(os.path.dirname(__file__), "..", "..", "TASKS.md")


def write_tasks_md() -> str:
    """Regenerate TASKS.md with current task state. Returns the file path."""
    today = date.today()
    week_ago = today - timedelta(days=7)

    # Fetch all templates
    templates = BusinessTask.query.order_by(
        BusinessTask.frequency, BusinessTask.priority
    ).all()

    # Fetch today's instances
    today_rows = db.session.query(TaskInstance, BusinessTask).join(
        BusinessTask, TaskInstance.task_id == BusinessTask.id
    ).filter(TaskInstance.due_date == today).order_by(BusinessTask.priority).all()

    # Fetch recent history (7 days)
    history_rows = db.session.query(TaskInstance, BusinessTask).join(
        BusinessTask, TaskInstance.task_id == BusinessTask.id
    ).filter(
        TaskInstance.due_date >= week_ago
    ).order_by(TaskInstance.due_date.desc(), BusinessTask.priority).all()

    lines = []
    lines.append("# TASKS.md — Business Task Log")
    lines.append("")
    lines.append(f"> Auto-generated: {datetime.utcnow().isoformat()}Z")
    lines.append("> This file is updated whenever tasks change. LLMs: read this to understand current operational state.")
    lines.append("")
    ...
    pipeline_logs = InsightLog.query.filter(

"""PetNet vendor integration."""
from typing import Any
import csv
from app.vendors.base import BaseVendor


class PetNetVendor(BaseVendor):
    SLUG = "petnet"
    DISPLAY_NAME = "PetNet"

    CSV_COLUMNS = {
        "sku": ["catalog #", "catalog number", "item code", "sku", "product code"],
        "description": ["description", "product name", "item name"],
        "quantity": ["qty", "quantity", "qty ordered"],
        "unit_price": ["unit price", "each", "price"],
        "line_total": ["amount", "total", "ext", "extended amount"],
    }
    ...
    def _parse_invoice_text(self, text: str) -> dict[str, Any]:
        ...
        m = re.search(r"PO\s*#?\s*[:\-]?\s*(\w+)", text, re.IGNORECASE)
        if m:

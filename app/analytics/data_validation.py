...
from app.analytics.public_code_tables import (
    VALID_CARC_CODES,
...
VALID_CLAIM_STATUSES_SET = VALID_CLAIM_STATUS_CODES
...
def validate_billing_record(record: dict) -> list[ValidationResult]:
    ...
    for field in ("patient_name", "referring_doctor", "insurance_carrier", "modality", "service_date"):
        ...
        if not val or (isinstance(val, str) and not val.strip()):
            results.append(ValidationResult(
                False, field, val, f"Required field '{field}' is missing or empty",
            ))

    name = record.get("patient_name", "")
    if name:
        if len(name) < 3:
            results.append(ValidationResult(False, "patient_name", name, "Patient name too short (< 3 chars)"))
        if name.replace(" ", "").isdigit():
            results.append(ValidationResult(False, "patient_name", name, "Patient name is numeric-only — likely a data error"))
    ...
    if svc_date and isinstance(svc_date, date):
        if svc_date > date.today():
            results.append(ValidationResult(False, "service_date", svc_date, "Service date is in the future"))
        if svc_date < date(2010, 1, 1):
            results.append(ValidationResult(False, "service_date", svc_date, "Service date before 2010 — likely a parsing error", severity="WARNING"))
    ...
    for field in ("primary_payment", "secondary_payment", "total_payment"):
        ...
        try:
            ...
            results.append(ValidationResult(False, field, val, f"Payment field '{field}' is not a valid number"))
            continue
        if fval < 0:
            results.append(ValidationResult(False, field, val, f"Payment '{field}' is negative: {fval}", severity="WARNING"))
        if fval > 100_000:
            results.append(ValidationResult(False, field, val, f"Payment '{field}' is unusually high: ${fval:,.2f} — verify", severity="WARNING"))
    ...
    if carrier:
        if len(carrier) < 2:
            results.append(ValidationResult(False, "insurance_carrier", carrier, "Insurance carrier code too short", severity="WARNING"))
        if carrier.upper() in ("X", "UNKNOWN", "N/A", "NA", "NONE", ""):
            results.append(ValidationResult(False, "insurance_carrier", carrier, f"Insurance carrier is placeholder '{carrier}' — needs verification", severity="WARNING"))
...
def validate_era_claim_line(record: dict) -> list[ValidationResult]:
    ...
    if status and status not in VALID_CLAIM_STATUS_CODES:
        results.append(ValidationResult(False, "claim_status", status, f"Unknown X12 claim status '{status}'. Valid codes: {', '.join(sorted(VALID_CLAIM_STATUS_CODES))}"))
    elif status:
        label = lookup_claim_status(status)
        if label and "Denied" in label:
            results.append(ValidationResult(True, "claim_status", status, f"Claim denied: {label}", severity="INFO"))

    group = record.get("cas_group_code")
    if group and group.upper() not in CAS_GROUP_CODES:
        results.append(ValidationResult(False, "cas_group_code", group, f"Unknown CAS group code '{group}'. Valid: {', '.join(sorted(CAS_GROUP_CODES))}", severity="WARNING"))

    reason = record.get("cas_reason_code")
    if reason:
        reason_clean = str(reason).strip()
        if reason_clean not in VALID_CARC_CODES:
            results.append(ValidationResult(False, "cas_reason_code", reason, f"Unknown CARC code '{reason}'. Not in X12 CARC standard (294 codes)", severity="WARNING"))
        else:
            desc = lookup_carc(reason_clean)
            if reason_clean in ("29", "27", "26"):
                results.append(ValidationResult(True, "cas_reason_code", reason, f"Filing/eligibility denial: CARC-{reason_clean} ({desc})", severity="INFO"))

    cpt = record.get("cpt_code")
    if cpt:
        clean = cpt.strip()
        if not is_valid_cpt_format(clean):
            results.append(ValidationResult(False, "cpt_code", cpt, f"'{cpt}' is not valid CPT (5 digits) or HCPCS Level II (letter + 4 digits) format", severity="WARNING"))
        elif clean.isdigit() and not is_radiology_cpt_range(clean):
            results.append(ValidationResult(False, "cpt_code", cpt, f"CPT '{cpt}' is outside radiology ranges (70010-76999, 77001-77799, 78000-79999)", severity="WARNING"))
            ...
            if expected_modality and claim_modality and expected_modality.upper() != claim_modality.upper():
                results.append(ValidationResult(False, "cpt_code", cpt, f"CPT {cpt} ({lookup_cpt(clean)}) maps to {expected_modality}, but claim modality is {claim_modality}", severity="WARNING"))

    payment_method = record.get("payment_method")
    if payment_method and payment_method not in VALID_PAYMENT_METHODS:
        results.append(ValidationResult(False, "payment_method", payment_method, f"Unknown payment method '{payment_method}'. Valid X12 BPR codes: {', '.join(sorted(VALID_PAYMENT_METHODS))}", severity="WARNING"))

    billed = float(record.get("billed_amount", 0) or 0)
    paid = float(record.get("paid_amount", 0) or 0)
    if paid > billed > 0:
        results.append(ValidationResult(False, "paid_amount", paid, f"Paid (${paid:,.2f}) exceeds billed (${billed:,.2f})", severity="WARNING"))
    ...
    if cas_adj > 0 and billed > 0 and paid >= 0:
        ...
        if expected_adj > 0 and abs(cas_adj - expected_adj) > 1.0:
            results.append(ValidationResult(False, "cas_adjustment_amount", cas_adj, f"CAS adjustment (${cas_adj:,.2f}) differs from billed-paid delta (${expected_adj:,.2f}) by more than $1.00", severity="INFO"))
...
def validate_era_payment(record: dict) -> list[ValidationResult]:
    ...
    if method and method not in VALID_PAYMENT_METHODS:
        results.append(ValidationResult(False, "payment_method", method, f"Unknown X12 payment method '{method}'. Valid: {', '.join(sorted(VALID_PAYMENT_METHODS))}", severity="WARNING"))
    ...
    if amount is not None:
        try:
            fval = float(amount)
            if fval < 0:
                results.append(ValidationResult(False, "payment_amount", amount, f"Negative ERA payment amount: ${fval:,.2f}", severity="WARNING"))
            if fval > 1_000_000:
                results.append(ValidationResult(False, "payment_amount", amount, f"Unusually large ERA payment: ${fval:,.2f} — verify", severity="WARNING"))
        except (ValueError, TypeError):
            results.append(ValidationResult(False, "payment_amount", amount, f"Payment amount is not a valid number: {amount}"))

    pdate = record.get("payment_date")
    if pdate and isinstance(pdate, date):
        if pdate > date.today():
            results.append(ValidationResult(False, "payment_date", pdate, "ERA payment date is in the future", severity="WARNING"))
        if pdate < date(2010, 1, 1):
            results.append(ValidationResult(False, "payment_date", pdate, "ERA payment date before 2010 — likely a parsing error", severity="WARNING"))

    check = record.get("check_eft_number")
    if check:
        check = str(check).strip()
        if len(check) < 2:
            results.append(ValidationResult(False, "check_eft_number", check, "Check/EFT number too short — likely incomplete", severity="WARNING"))
...
def validate_payer_code(code: str, known_payers: set[str]) -> ValidationResult:
    ...
    return ValidationResult(False, "insurance_carrier", code, f"Payer '{code}' not in registry — add to payers table or verify spelling", severity="WARNING")
...
def validate_batch(records: list[dict], known_payers: set[str] | None = None) -> dict:
    ...
    return {
        "total": total, "valid": valid, "warnings": warnings, "errors": errors,
        "error_details": error_details[:50], "warning_details": warning_details[:50],
        "unknown_payers": sorted(unknown_payers), "unknown_modalities": sorted(unknown_modalities),
    }
...
def validate_era_batch(claims: list[dict], payment_info: dict | None = None) -> dict:
    ...
    return {
        ...
        "claim_errors": claim_errors[:50], "claim_warnings": claim_warnings[:50],
